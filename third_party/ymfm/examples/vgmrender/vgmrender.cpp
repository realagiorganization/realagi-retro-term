...
std::atomic_bool *s_cancel_flag = nullptr;
...
void generate_all(std::vector<uint8_t> &buffer, uint32_t data_start, uint32_t output_rate, std::vector<int32_t> &wav_buffer)
{
	...
	while (!done && offset < buffer.size() && (s_cancel_flag == nullptr || !s_cancel_flag->load()))
	{
		int delay = 0;
		...
		while (delay-- != 0)
		{
			if (s_cancel_flag != nullptr && s_cancel_flag->load())
			{
				done = true;
				break;
			}
...
int ymfm_vgmrender_file(char const *filename, char const *outfilename, int output_rate, std::atomic_bool *cancel_flag)
{
	if (filename == nullptr || outfilename == nullptr)
		return 1;
	if (output_rate <= 0)
		output_rate = 44100;

	s_cancel_flag = cancel_flag;
	active_chips.clear();
	...
	if (buffer.size() >= 10 && buffer[0] == 0x1f && buffer[1] == 0x8b && buffer[2] == 0x08)
	{
		...
		if (uncompressed < compressed.size() || uncompressed > 32 * 1024 * 1024)
		{
			s_cancel_flag = nullptr;
			return 4;
		}
